...
async def get_picture_id(cursor, user_id, image, container_client):
    ...
    try:
        ...
        return await nachet_datastore.upload_picture_unknown(
            cursor, str(user_id), image, container_client
        )
    except Exception as error:
        raise DatastoreError(error)
...
async def upload_pictures(
    ...
    try:
        return await nachet_datastore.upload_pictures(
            cursor,
            user_id,
            picture_set_id,
            container_client,
...
async def create_picture_set(
    cursor, container_client, user_id: str, nb_pictures: int, folder_name=None
):
    try:
        return await datastore.create_picture_set(
            cursor, container_client, nb_pictures, user_id, folder_name
        )
    except Exception as error:
        raise DatastoreError(error)
...
async def save_inference_result(
    cursor, user_id: str, inference_dict, picture_id: str, pipeline_id: str, type: int
):
    try:
        return await nachet_datastore.register_inference_result(
            cursor, user_id, inference_dict, picture_id, pipeline_id, type
        )
    except Exception as error:
        raise DatastoreError(error)
...
async def save_perfect_feedback(cursor, inference_id: str, user_id: str, boxes_id):
    try:
        await nachet_datastore.new_perfect_inference_feeback(
...
async def delete_directory_request(cursor, user_id, picture_set_id):
    try:
        return (
            len(
                await nachet_datastore.find_validated_pictures(
                    cursor, user_id, picture_set_id
                )
            )
            > 0
        )
...
async def delete_directory_permanently(
    cursor, user_id, picture_set_id, container_client
):
    try:
        return await datastore.delete_picture_set_permanently(
            cursor, user_id, picture_set_id, container_client
        )
    except Exception as error:
        raise DatastoreError(error)
...
async def delete_directory_with_archive(
    cursor, user_id, picture_set_id, container_client
):
    try:
        return await nachet_datastore.delete_picture_set_with_archive(
            cursor, user_id, picture_set_id, container_client
        )
    except Exception as error:
        raise DatastoreError(error)
...
async def get_inference(cursor, user_id, picture_id=None, inference_id=None):
    try:
        return await nachet_datastore.get_picture_inference(
            cursor, user_id, picture_id, inference_id
        )
    except Exception as error:
        raise DatastoreError(error)
...
async def get_picture_blob(cursor, user_id, container_client, picture_id):
    try:
        return await nachet_datastore.get_picture_blob(
            cursor, user_id, container_client, picture_id
        )
    except Exception as error:
        raise DatastoreError(error)
