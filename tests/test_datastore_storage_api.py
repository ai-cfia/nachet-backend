...
class TestConnection(unittest.TestCase):
    ...
    @patch(
        "storage.datastore_storage_api.NACHET_DB_CONN_URL", "postgresql://invalid_url"
    )
...
class TestSeedsGetters(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_all_seeds_successful(self):
        ...
        with (
            patch(
                "storage.datastore_storage_api.get_connection",
            ...
            with patch(
                "storage.datastore_storage_api.nachet_datastore.get_seed_info",
                new=mock_get_seed_info,
            ):
    ...
    def test_get_all_seeds_names_successful(self):
        ...
        with (
            ...
            with patch(
                "storage.datastore_storage_api.seed_queries.get_all_seeds_names",
                new=mock_get_all_seeds_names,
            ):
    ...
    def test_get_all_seeds_names_error(self):
        ...
        with (
            ...
            mock_get_all_seeds_names = MagicMock(
                side_effect=Exception("Seed not found")
            )
...
class TestUser(unittest.IsolatedAsyncioTestCase):
    ...
    def test_get_user_id_successful(self):
        ...
        with (
            ...
            patch(
                "storage.datastore_storage_api.user_datastore.is_user_registered",
                new=mock_is_user_registered,
            ),
            patch(
                "storage.datastore_storage_api.user_datastore.get_user_id",
                new=mock_get_user_id,
            ),
    ...
    async def test_create_user_successful(self):
        mock_new_user = AsyncMock(
            return_value=datastore.datastore.User(self.email, self.user_id)
        )
        ...
        with (
            ...
            patch("storage.datastore_storage_api.get_cursor") as mock_get_cursor,
            patch(
                "storage.datastore_storage_api.datastore.new_user", new=mock_new_user
            ),
            patch("storage.datastore_storage_api.end_query") as mock_end_query,
        ):
            mock_connection = mock_get_connection.return_value
            ...
            mock_new_user.assert_awaited_once_with(
                mock_cursor, self.email, self.connection_string
            )
...
class TestPicture(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_picture_id_successful(self):
        ...
        with patch(
            "storage.datastore_storage_api.nachet_datastore.upload_picture_unknown",
            new=mock_upload_picture_unknown,
        ):
            self.assertEqual(
                str(
                    await datastore.get_picture_id(
                        self.mock_cursor,
                        self.user_id,
                        self.mock_image,
            ...
            mock_upload_picture_unknown.assert_awaited_once_with(
                self.mock_cursor,
                self.user_id,
                self.mock_image,
                self.mock_container_client,
    ...
    async def test_get_picture_id_error(self):
        mock_upload_picture_unknown = AsyncMock(
            side_effect=Exception("User not found error")
        )
        with patch(
            "storage.datastore_storage_api.nachet_datastore.upload_picture_unknown",
            new=mock_upload_picture_unknown,
    ...
    async def test_upload_pictures_successful(self):
        ...
        with patch(
            "storage.datastore_storage_api.nachet_datastore.upload_pictures",
            new=mock_upload_pictures,
        ):
            self.assertEqual(
            ...
            mock_upload_pictures.assert_awaited_once_with(
                self.mock_cursor,
                self.user_id,
                self.picture_set_id,
                self.mock_container_client,
    ...
    async def test_create_picture_set_successful(self):
        ...
        with patch(
            "storage.datastore_storage_api.datastore.create_picture_set",
            new=mock_create_picture_set,
        ):
            self.assertEqual(
            ...
            mock_create_picture_set.assert_awaited_once_with(
                self.mock_cursor,
                self.mock_container_client,
                len([self.mock_image]),
                self.user_id,
    ...
    async def test_create_picture_set_error(self):
        mock_create_picture_set = AsyncMock(
            side_effect=Exception("User not found error")
        )
        with patch(
            "storage.datastore_storage_api.datastore.create_picture_set",
...
class TestPipelines(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_pipelines_successful(self):
        mock_get_ml_structure = AsyncMock(
            return_value=[{"pipeline_id": self.test_pipeline_id}]
        )
        with (
            patch(
                "storage.datastore_storage_api.get_connection"
            ...
            patch("storage.datastore_storage_api.get_cursor") as mock_get_cursor,
            patch(
                "storage.datastore_storage_api.nachet_datastore.get_ml_structure",
                new=mock_get_ml_structure,
            ),
        ):
            mock_connection = mock_get_connection.return_value
            mock_cursor = mock_get_cursor.return_value
    ...
    async def test_get_pipelines_error(self):
        mock_get_ml_structure = AsyncMock(
            side_effect=Exception("Pipeline retrieval failed")
        )
        with (
            patch("storage.datastore_storage_api.get_connection"),
...
class TestSaveInferenceResult(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_save_inference_result_successful(self):
        with patch(
            "storage.datastore_storage_api.nachet_datastore.register_inference_result",
            new_callable=AsyncMock,
        ) as mock_register_inference_result:
            mock_register_inference_result.return_value = self.test_inference_dict
            result = await datastore.save_inference_result(
                self.mock_cursor,
                self.test_user_id,
                self.test_inference_dict,
                self.test_picture_id,
    ...
    async def test_save_inference_result_error(self):
        with patch(
            ...
            mock_register_inference_result.side_effect = Exception(
                "Save inference failed"
            )
            with self.assertRaises(datastore.DatastoreError):
...
class TestSaveFeedback(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_save_perfect_feedback_successful(self):
        with patch(
            "storage.datastore_storage_api.nachet_datastore.new_perfect_inference_feeback",
            new_callable=AsyncMock,
        ) as mock_new_perfect_feedback:
            await datastore.save_perfect_feedback(
                self.mock_cursor,
                self.test_inference_id,
                self.test_user_id,
                self.test_boxes_id,
            )
    ...
    async def test_save_perfect_feedback_error(self):
        with patch(
            ...
            mock_new_perfect_feedback.side_effect = Exception(
                "Save perfect feedback failed"
            )
            with self.assertRaises(datastore.DatastoreError):
                await datastore.save_perfect_feedback(
                    self.mock_cursor,
    ...
    async def test_save_annoted_feedback_successful(self):
        with patch(
            "storage.datastore_storage_api.nachet_datastore.new_correction_inference_feedback",
            new_callable=AsyncMock,
        ) as mock_new_correction_feedback:
    ...
    async def test_save_annoted_feedback_error(self):
        with patch(
            ...
            mock_new_correction_feedback.side_effect = Exception(
                "Save annoted feedback failed"
            )
            with self.assertRaises(datastore.DatastoreError):
                await datastore.save_annoted_feedback(
                    self.mock_cursor, self.test_feedback_dict
...
class TestDeleteDirectories(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_delete_directory_request_successful(self):
        with patch(
            "storage.datastore_storage_api.nachet_datastore.find_validated_pictures",
            new_callable=AsyncMock,
        ) as mock_find_validated_pictures:
            mock_find_validated_pictures.return_value = self.test_validated_pictures
            self.assertTrue(
                await datastore.delete_directory_request(
                    self.mock_cursor, self.test_user_id, self.test_picture_set_id
                )
            )
    ...
    async def test_delete_directory_request_error(self):
        with patch(
            ...
            mock_find_validated_pictures.side_effect = Exception(
                "Search for validated pictures failed"
            )
            with self.assertRaises(datastore.DatastoreError):
                await datastore.delete_directory_request(
                    self.mock_cursor, self.test_user_id, self.test_picture_set_id
    ...
    async def test_delete_directory_permanently_successful(self):
        with patch(
            "storage.datastore_storage_api.datastore.delete_picture_set_permanently",
            new_callable=AsyncMock,
        ) as mock_delete_permanently:
            mock_delete_permanently.return_value = True
            self.assertTrue(
                await datastore.delete_directory_permanently(
                    self.mock_cursor,
                    self.test_user_id,
    ...
    async def test_delete_directory_permanently_error(self):
        with patch(
            ...
            mock_delete_permanently.side_effect = Exception(
                "Search for validated pictures failed"
            )
            with self.assertRaises(datastore.DatastoreError):
                await datastore.delete_directory_permanently(
    ...
    async def test_delete_directory_with_archive_successful(self):
        with patch(
            "storage.datastore_storage_api.nachet_datastore.delete_picture_set_with_archive",
            new_callable=AsyncMock,
        ) as mock_delete_with_archive:
            mock_delete_with_archive.return_value = True
            self.assertTrue(
                await datastore.delete_directory_with_archive(
                    self.mock_cursor,
                    self.test_user_id,
                    self.test_picture_set_id,
                    self.mock_container_client,
    ...
    async def test_delete_directory_with_archive_error(self):
        with patch(
            ...
            mock_delete_with_archive.side_effect = Exception(
                "Search for validated pictures failed"
            )
            with self.assertRaises(datastore.DatastoreError):
                await datastore.delete_directory_with_archive(
...
class TestGetDirectories(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.test_picture_set_id = "test_picture_set_id"
        self.folder_name = "test_folder_name"
        self.test_picture_sets = [
            {
                "picture_set_id": self.test_picture_set_id,
    ...
    async def test_get_directories_successful(self):
        with patch(
            "storage.datastore_storage_api.nachet_datastore.get_picture_sets_info",
            new_callable=AsyncMock,
        ) as mock_get_picture_sets_info:
            mock_get_picture_sets_info.return_value = self.test_picture_sets
            directories_info = await datastore.get_directories(
    ...
    async def test_get_directories_error(self):
        with patch(
            ...
            mock_get_picture_sets_info.side_effect = Exception(
                "Failed to retrieve directories information"
            )
            with self.assertRaises(datastore.DatastoreError):
...
class TestGetInference(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_inference_successful(self):
        with patch(
            "storage.datastore_storage_api.nachet_datastore.get_picture_inference",
            new_callable=AsyncMock,
        ) as mock_get_inference:
            mock_get_inference.return_value = self.test_inference_dict

            result = await datastore.get_inference(
                self.mock_cursor, self.test_user_id, self.test_picture_id
            )
            self.assertEqual(result, self.test_inference_dict)
            mock_get_inference.assert_awaited_once_with(
                self.mock_cursor, self.test_user_id, self.test_picture_id, None
            )

            result = await datastore.get_inference(
                self.mock_cursor, self.test_user_id, inference_id=self.test_inference_id
            )
            self.assertEqual(result, self.test_inference_dict)
            mock_get_inference.assert_awaited_with(
                self.mock_cursor, self.test_user_id, None, self.test_inference_id
            )
    ...
    async def test_get_inference_error(self):
        with patch(
            ...
            mock_get_inference.side_effect = Exception(
                "Failed to retrieve inference information"
            )
            with self.assertRaises(datastore.DatastoreError):
                await datastore.get_inference(
                    self.mock_cursor, self.test_user_id, self.test_picture_id
                )
...
class TestGetPictureBlob(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_picture_blob_successful(self):
        with patch(
            "storage.datastore_storage_api.nachet_datastore.get_picture_blob",
            new_callable=AsyncMock,
        ) as mock_get_picture_blob:
            mock_get_picture_blob.return_value = self.mock_blob
            result = await datastore.get_picture_blob(
                self.mock_cursor,
                self.test_user_id,
                self.mock_container_client,
                self.test_picture_id,
    ...
    async def test_get_picture_blob_error(self):
        with patch(
            ...
            mock_get_picture_blob.side_effect = Exception(
                "Failed to retrieve directories information"
            )
            with self.assertRaises(datastore.DatastoreError):
                await datastore.get_picture_blob(
                    self.mock_cursor,
                    self.test_user_id,
                    self.mock_container_client,
                    self.test_picture_id,
